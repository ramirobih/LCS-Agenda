...
{
	private static final String insert = "INSERT INTO personas(idPersona, nombre, telefono, email, fechanacimiento, calle, altura, piso, depto, idLocalidad, idTipoContacto, idTipoDomicilio) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?)";
	private static final String update = "UPDATE personas SET nombre = ?, telefono = ?, email = ?, fechanacimiento = ?, calle = ?, altura = ?, piso = ?, depto = ?, idLocalidad = ?, idTipoContacto = ?, idTipoDomicilio = ? WHERE idPersona = ?";
	...
	private static final String readAllWithJoins = 
			...
			" l.idLocalidad,l.Provincia,l.Pais, l.nombre as localidadNombre, t.idTipoContacto, t.nombre as TipoContactoNombre, d.idTipoDomicilio, d.nombre as TipoDomicilioNombre " + 
			"FROM personas as p "+
			"INNER JOIN localidades as l ON p.idLocalidad = l.idLocalidad " + 
			"INNER JOIN tiposContacto as t ON p.idTipoContacto = t.idTipoContacto " +
			"INNER JOIN tiposDomicilio as d ON p.idTipoDomicilio = d.idTipoDomicilio";
	...
	public boolean insert(PersonaDTO persona)
	{
		...
		try 
		{
			...
			statement.setInt(12, persona.getTipoDomicilio().getIdTipoDomicilio());
	...
	public boolean update(PersonaDTO persona) 
	{
		...
		try 
		{
			...
			statement.setInt(11, persona.getTipoDomicilio().getIdTipoDomicilio());
	...
	public List<PersonaDTO> readAll()
	{
		...
		try 
		{
			...
			while(resultSet.next())
			{
				...
				TipoDomicilioDTO tipoDomicilio = new TipoDomicilioDTO(resultSet.getInt("idTipoDomicilio"),resultSet.getString("tipoDomicilioNombre"));				
				...
				personas.add(new PersonaDTO(resultSet.getInt("idPersona"), resultSet.getString("Nombre"), resultSet.getString("Telefono"),
						...
						localidad,tipoContacto,tipoDomicilio));
			}
		} 
		catch (SQLException e) 
		{
